...
def show(
    ...
    general: bool = typer.Option(False, help="Show general configuration options available"),
    ach: bool = typer.Option(False, help="Show required ACH calculation options available."),
    ashrae: bool = typer.Option(False, help="Show ASHRAE calculation options available."),
...
def general(
    ...
    ashrae: bool = typer.Option(settings["general"]["ashrae"], help="Set whether run command makes ventilation calculation requirements according to ASHRAE recomendations with default values"),
...
@config_app.command()
def required_ach(
    max_risk: float = typer.Option(settings["ach"]["max_risk"], min=0, max=100, help="Set maximum risk for ACH calculations"),
...
def ashrae(
    ...
    settings["ashare"][f"{key_name}"] = {"rate_people": rate_people, "rate_area": rate_area}
...
def graphics(
    ...
    color_scheme: List[str] = typer.Option(settings["graphics"]["color_scheme"], help="List of colors to use in graphics"),
    ...
    scale: int = typer.Option(settings["graphics"]["scale"], help="Scale for pictures, a higher number makes for better resolution, but it also increases image size")
) -> None:
    """
